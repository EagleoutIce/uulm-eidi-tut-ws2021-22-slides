...
\begin{frame}[c]{Übungsblatt 10 - Aufgabe 1\hfill Quicksort -- Vorlesungstreue}
    ...
    \end{enumerate}
    \begin{center}
        \onslide<10->{\info{Der \say{gleich}-Fall ist an sich beliebig. Man kann also auch \T{l} für kleiner gleich und \T{r} für größer als das Pivotelement verwenden. Man muss nur eine der Varianten wählen.}}
    \end{center}
\begin{tikzpicture}[overlay,remember picture]
    \onslide<9->{\node[left=-7mm,scale=.8] at(current page.10) {\usebox\pinguexplainbox};}
\end{tikzpicture}
\end{frame}
