...
\begin{frame}[t,fragile]{Übungsblatt 5 - Aufgabe 1 a)}
    \begin{itemize}[<+(1)->]
        \item Der volle Code befindet sich hier: \only<2->{\textattachfile{\curpath DotProduct.java}{DotProduct.java}}.
...
!*\onslide<6->*!}
!*\onslide<5->*!
!*\onslide<7->*!System.out.println("Vector1: " + vectorToString(vector1));
!*\onslide<7->*!System.out.println("Vector2: " + vectorToString(vector2));
!*\onslide<5->*!
!*\onslide<8->*!System.out.println("Das Skalarprodukt ist: " + d);
\end{plainjava}
    \end{itemize}
\end{frame}
...
\begin{plainjava}
!*\onslide<3->*!public static String vectorToString(int[] vector) {
!*\onslide<4->*!    String result = "(";
!*\onslide<5->*!    for (int i = 0; i < vector.length; i++) {
!*\onslide<6->*!        if (i > 0) result += ", ";
!*\onslide<7->*!        result += vector[i];
!*\onslide<5->*!    }
...
\iffull
\rExecute{javac DotProduct.java}
\begin{frame}[t,fragile]{Übungsblatt 5 - Aufgabe 1 a)}
    \begin{itemize}[<+(1)->]
        \item Als beispielhafte Ausgabe für \rbash[:\pause]{java DotProduct}
    \end{itemize}
\end{frame}
\fi
...
\begin{frame}[t,fragile]{Übungsblatt 5 - Aufgabe 1 b)}
    \begin{itemize}[<+(1)->]
        \item Der volle Code befindet sich hier: \only<2->{\textattachfile{\curpath Transpose.java}{Transpose.java}}.
...
\iffull
\rExecute{javac Transpose.java}
\begin{frame}[t,fragile]{Übungsblatt 5 - Aufgabe 1 b)}
    \def\rbashcurrentinject{multicols=2}%
    \begin{itemize}[<+(1)->]
        \item \lstfs{9}Als beispielhafte Ausgabe für \rbash[:\pause]{java Transpose}
    \end{itemize}
\end{frame}
\fi
...
\begin{frame}[t,fragile]{Übungsblatt 5 - Aufgabe 2}
    \begin{itemize}[<+(1)->]
        \item Der volle Code befindet sich hier: \only<2->{\textattachfile{\curpath MatrixMultiplication.java}{MatrixMultiplication.java}}.
...
\begin{frame}[t,fragile]{Übungsblatt 5 - Aufgabe 2}
    \begin{itemize}[<+(1)->]
        ...
        \onslide<3->{\(c_{i, j} = \sum_{k = 1}^m a_{i, k} \cdot b_{k, j}\).}\lstfs{10}
\begin{plainjava}
...
\iffull
\rExecute{javac MatrixMultiplication.java}
\begin{frame}[t,fragile]{Übungsblatt 5 - Aufgabe 2}
    \def\rbashcurrentinject{multicols=2}%
    \begin{itemize}[<+(1)->]
        \item \lstfs{8}Als beispielhafte (formatierte) Ausgabe für \rbash[:\pause]{java MatrixMultiplication}
    \end{itemize}
\end{frame}
\fi

\subsection{Zusatzaufgabe 2/6}
\begin{frame}[fragile]{Übungsblatt 5 - Zusatzaufgabe a)}
    \begin{itemize}[<+(1)->]
        \item Wir machen eine Raute (\only<2->{\textattachfile{\curpath Rhombus.java}{Rhombus.java}}).
...
\begin{frame}[fragile]{Übungsblatt 5 - Zusatzaufgabe a)}
    \begin{itemize}[<+(1)->]
        \item Negative Zahlen erlauben es, beide Hälften in einer Schleife zu behandeln:
\begin{plainjava}
!*\onslide<3->*!for(int i = N-1; i > -N; i--) {
!*\onslide<4->*!    for(int j = 1; j <= Math.abs(i); j++)
!*\onslide<5->*!        System.out.print(" ");
!*\onslide<3->*!
!*\onslide<6->*!    for(int j = 1; j <= N - Math.abs(i); j++) {
!*\onslide<7->*!        if((j == 1) || (j == N - Math.abs(i)))
!*\onslide<8->*!            System.out.print("* ");
!*\onslide<7->*!        else
!*\onslide<8->*!            System.out.print("  ");
!*\onslide<6->*!    }
!*\onslide<9->*!    System.out.println("");
...
\end{plainjava}
    \end{itemize}
\onslide<10->{\begin{tikzpicture}[overlay,remember picture]
    \node[above left=.55cm,xshift=.25cm] at(current page.south east) {\only<10->{\textattachfile{\curpath RhombusNegative.java}{RhombusNegative.java}}};
\end{tikzpicture}}
\end{frame}

\iffull
\rExecute{javac RhombusNegative.java}
\begin{frame}[t,fragile]{Übungsblatt 5 - Zusatzaufgabe a)}
    \lstfs{9}\begin{itemize}[<+(1)->]
        \item So ergibt sich beispielsweise \rbash[:\pause]{java RhombusNegative 9}
    \end{itemize}
\end{frame}
...
\scope[every node/.append style={below left,scale=.8,outer xsep=0pt,inner xsep=0pt,major}]
    ...
    \node (4) at (3.south east) {\(\abs{x} + \abs{y} \leq 4\)};
...
!*\onslide<10->*!for (int y = N; y >= -N; y--) {
!*\onslide<11->*!    for (int x = -N; x <= N; x++) {
...
\onslide<15->{\begin{tikzpicture}[overlay,remember picture]
    \node[above left=.55cm,xshift=.25cm] at(current page.south east) {\only<15->{\textattachfile{\curpath RhombusElegant.java}{RhombusElegant.java}}};
\end{tikzpicture}}
    \end{itemize}
\end{frame}
...
\begin{frame}[fragile]{Übungsblatt 5 - Zusatzaufgabe b)}
    \begin{itemize}[<+(1)->]
        \item Die Musterlösung (\only<2->{\textattachfile{\curpath NestedRhombus.java}{NestedRhombus.java}}).\lstfs{7}
\begin{plainjava}
...
!*\onslide<8->*!
!*\onslide<9->*!for (int y = N; y >= -N; y--) {
!*\onslide<10->*!    for (int x = -N; x <= N; x++) {
...
\endcolumns
\onslide<15->{\begin{tikzpicture}[overlay,remember picture]
    \node[above left=.55cm,xshift=.25cm] at(current page.south east) {\only<15->{\textattachfile{\curpath NestedRhombusElegant.java}{NestedRhombusElegant.java}}};
\end{tikzpicture}}
    \end{itemize}
\end{frame}
...
\begin{frame}[fragile]{Can we do it in one line?}
    \begin{itemize}[<+(1)->]
        \item \textbf{Nicht klausurrelevant}, aber durch Anfrage (\only<2->{\textattachfile{\curpath NestedRhombusOneLine.java}{NestedRhombusOneLine.java}}):
\lstfs{8}
\begin{plainjava}
...
!*\onslide<5->*!    .boxed()
!*\onslide<6->*!    .map(s -> Map.entry(s, !*\onslide<7->*!IntStream.rangeClosed(
!*\onslide<7->*!           -Integer.parseInt(args[0]) + 1,
!*\onslide<7->*!           Integer.parseInt(args[0]) - 1).boxed()!*\onslide<6->*!))
!*\onslide<7->*!    .map(s -> s.getValue()
!*\onslide<8->*!        .map(x -> !*\onslide<9->*!Math.abs(s.getKey()) + Math.abs(x) <= (Integer.parseInt(args[0]) - 1) &&
!*\onslide<9->*!            (Math.abs(s.getKey()) + Math.abs(x)) % 4 == (Integer.parseInt(args[0]) - 1) % 4
!*\onslide<9->*!            ? "*" : " "
!*\onslide<8->*!        )!*\onslide<10->*!.reduce("", (a, b) -> a + b)
!*\onslide<7->*!    )
!*\onslide<11->*!    .collect(Collectors.joining("\n"))
!*\onslide<3->*!);
\end{plainjava}
    \end{itemize}
\end{frame}
...
\begin{frame}[fragile]{Übungsblatt 5 - Zusatzaufgabe\hfill Weiter Kommentare}
    \begin{itemize}[<+(1)->]
        \itemsep6pt
        \item Man kann das Problem auch leicht rekursiv lösen. Das war so aber nicht gedacht.\pause \infoblock{Man kann jede Zeile auf die Basisfälle \say{Nichts}, \say{\bjava{*}}, \say{\bjava{* *}},~\ldots\ reduzieren und somit rekursiv bauen. Auch ne nette one-liner Idee\ldots\ Was sind aber die Nachteile der Rekursion?}
        ...
        \item Code-Kommentare sollten nicht redundanterweise erklären, was die jeweilige Zeile macht, sondern maximal was die Idee/das Konzept dahinter war. Kommentare sind an sich auch Code! Nur für menschliche \say{Compiler}. \info{Oder sowas wie GPT-3.}
    \end{itemize}
\end{frame}
\fi
...
\begin{frame}[fragile]{Vectori-bras, the great solver}
    \begin{itemize}[<+(1)->]
        \item Hier die Klasse an sich: \only<2->{\textattachfile{\curpath Vector.java}{Vector.java}}
...
\begin{frame}[fragile]{Alte Zusatzaufgabe}
    \begin{itemize}[<+(1)->]
        \item Für Türme lässt sich die Aufgabe leicht umformulieren (\only<2->{\textattachfile{\curpath Zusatz.java}{Zusatz.java}}):\par
...
\begin{frame}[fragile]{Alte Zusatzaufgabe, Alternative}
    \begin{itemize}[<+(1)->]
        \item Betrachten wir eine Prüfroutine für ein gegebenes Feld (\only<2->{\textattachfile{\curpath ZusatzB.java}{ZusatzB.java}})
\begin{plainjava}
...
\begin{frame}[t,fragile]{Beispiele zur Überladung}
    \begin{itemize}[<+->]
        \item Es sei (die volle Datei: \only<1->{\textattachfile{\curpath Ueberladen.java}{Ueberladen.java}}):\pause{}
\begin{java}
        ...
        \begin{center}
            \begin{tikzpicture}
                ...
                \begin{scope}[every node/.append style={scale=0.7}]
                    \onslide<6->{
                        \draw[decoration={brace,mirror},decorate] (n.166)++(1pt,0) -- ++(-13pt,0) node[midway,above=3pt] {Array};
                    }
                    \onslide<7->{
                        \draw[decoration=brace,decorate] (n.190)++(2pt,0) -- ++(-17pt,0) node[midway,below=3pt] {Integer};
                    }
                    % position marker
                    \onslide<8->{
                    ...
                    \onslide<9->{
                        \draw[Kite-] (n.344) -- ++(    0,-0.35) node[below] {\bjava{arr|custom|[1]|custom|}};
                    }
                    \onslide<10->{
                        \draw[Kite-] (n.350) -- ++( 0.45,-0.35) node[below] {\bjava{arr|custom|[2]|custom|}};
                    }
                    %length
                    \onslide<11->{
                        \draw[decoration={brace,mirror},decorate] (n.8)++(-4pt,0) -- ++(-49pt,0) node[midway,above=3pt] {\bjava{arr.length :yields: 3}};
                    }
                \end{scope}
            \end{tikzpicture}
        \end{center}
    ...
    \begin{itemize}[<+(1)->]
        ...
        \begin{center}
            \begin{tikzpicture}
                ...
                \begin{scope}[every node/.append style={scale=0.7}]
                    \onslide<6->{
                        \draw[decoration={brace,mirror},decorate] (n.169)++(-9pt,0) -- ++(-14pt,0) node[midway,above=3pt] {Array};
                    }
                    \onslide<7->{
                        \draw[decoration=brace,decorate] (n.186)++(14pt,0) -- ++(-29pt,0) node[midway,below=3pt] {Arrays};
                    }
                    % position marker
                    \onslide<8->{
                    ...
                    \onslide<9->{
                        \draw[Kite-] (n.350)++(16pt,0) -- ++(    0,-0.35) node[below] (wa) {\bjava{arr|custom|[1]|custom|}};
                    }
                    %length
                    \onslide<10->{
                        \draw[decoration={brace,mirror},decorate] (n.7)++(18.5pt,0) -- ++(-103pt,0) node[midway,above=3pt] {\bjava{arr.length :yields: 2}};
                    }
                    \onslide<11->{
        ...
        \item[\color{cprimary}\tiny\raisebox{1.45pt}{$\blacksquare$}]<18-> Vorerst: \onslide<19->{\bjava{arr.length} = Anzahl Zeilen} \onslide<20->{und} \onslide<21->{\bjava{arr[0].length} = Anzahl Spalten.}
    \end{itemize}
\end{frame}
            ...
            \begin{tikzpicture}
                \node at(0,0) {\(\displaystyle\begin{array}{ccc}
                    ...
                    \end{pmatrix} & {\color{gray}\begin{pmatrix}
                      \only<1-6|handout:0>{\buff{\mathbf{a}}}\only<7->{\buff{\mathbf{9}}}&\buff{b}\\
                      \buff{c}&\buff{d}
                    \end{pmatrix}} \\
              ...
              \onslide<3->{
                \draw[Kite-Kite,color=paletteA] (-2.125+\shift,-0.35) to[bend left, edge node={node[above,sloped] {\(\cdot\)}}] (0.8-\nshift,1.65);
              }
              \onslide<4->{
                \draw[Kite-Kite,color=paletteB] (-1.5+\shift,-0.35) to[bend left, edge node={node[above,sloped] {\(\cdot\)}}] (0.8-\nshift,1.15);
              }
              \onslide<5->{
                \draw[Kite-Kite,color=paletteC] (-0.99+\shift,-0.35) to[bend left, edge node={node[above,sloped] {\(\cdot\)}}] (0.8-\nshift,0.65);
              }
              \onslide<6->{
                \draw[Kite-Kite,color=paletteD] (-0.35+\shift,-0.35) to[bend left, edge node={node[above,sloped] {\(\cdot\)}}] (0.8-\nshift,0.25);
              }
            ...
            \begin{tikzpicture}
                \node at(0,0) {\(\displaystyle\begin{array}{ccc}
                    ...
                    \begin{pmatrix}
                        ...
                        1 & 1 & 3 & 0
                    \end{pmatrix} & {\color{gray}\begin{pmatrix}
                        \buff{9}&\only<1-6|handout:0>{\buff{\mathbf{b}}}\only<7->{\buff{\mathbf{8}}}\\
                        \buff{c}&\buff{d}
                    \end{pmatrix}} \\
                    \fakepad{\textcolor{cprimary}{A}}
                \end{array}\)};
              ...
              \draw[dashed,thin, color=lightgray] (1.05,2) -- ++(0,-3.5);
              \onslide<3->{
                \draw[Kite-Kite,color=paletteA] (-2.125+\shift,-0.35) to[bend left, edge node={node[above,sloped] {\(\cdot\)}}] (1.325-\nshift,1.65);
              }
              \onslide<4->{
                \draw[Kite-Kite,color=paletteB] (-1.5+\shift,-0.35) to[bend left, edge node={node[above,sloped] {\(\cdot\)}}] (1.325-\nshift,1.15);
              }
              \onslide<5->{
                \draw[Kite-Kite,color=paletteC] (-0.99+\shift,-0.35) to[bend left, edge node={node[above,sloped] {\(\cdot\)}}] (1.325-\nshift,0.65);
              }
              \onslide<6->{
                \draw[Kite-Kite,color=paletteD] (-0.35+\shift,-0.35) to[bend left, edge node={node[above,sloped] {\(\cdot\)}}] (1.325-\nshift,0.25);
              }
              \node at(6.2,0) {\begin{minipage}{0.4\linewidth}
        ...
        \item<1-> \(A \cdot B\):\pause{} \begin{center}
            \begin{tikzpicture}
                \node at(0,0) {\(\displaystyle\begin{array}{ccc}
                    ...
                    \end{pmatrix} & {\color{gray}\begin{pmatrix}
                        \buff{9}&\buff{8}\\
                      \only<1-6|handout:0>{\buff{\mathbf{c}}}\only<7->{\buff{\mathbf{7}}}&\buff{d}
                    \end{pmatrix}} \\
                    \fakepad{\textcolor{cprimary}{A}}
                \end{array}\)};
                \draw[dashed,thin, color=lightgray] (-2.65,-0.79) -- ++(4.65,0);
              \draw[dashed,thin, color=lightgray] (1.05,2) -- ++(0,-3.5);
              \onslide<3->{
                \draw[Kite-Kite,color=paletteA] (-2.125+\shift,-0.75) to[bend left, edge node={node[above,sloped] {\(\cdot\)}}] (0.8-\nshift,1.65);
              }
              \onslide<4->{
                \draw[Kite-Kite,color=paletteB] (-1.5+\shift,-0.75) to[bend left, edge node={node[above,sloped] {\(\cdot\)}}] (0.8-\nshift,1.15);
              }
              \onslide<5->{
                \draw[Kite-Kite,color=paletteC] (-0.99+\shift,-0.75) to[bend left, edge node={node[above,sloped] {\(\cdot\)}}] (0.8-\nshift,0.65);
              }
              \onslide<6->{
                \draw[Kite-Kite,color=paletteD] (-0.35+\shift,-0.75) to[bend left, edge node={node[above,sloped] {\(\cdot\)}}] (0.8-\nshift,0.25);
              }
              \node at(6.2,0) {\begin{minipage}{0.4\linewidth}
            ...
            \begin{tikzpicture}
                \node at(0,0) {\(\displaystyle\begin{array}{ccc}
                    ...
                    \end{pmatrix} & {\only<1-7>{\color{gray}}\begin{pmatrix}
                      \buff{9}&\buff{8}\\
                      \buff{7}&\only<1-6|handout:0>{\buff{\mathbf{d}}}\only<7>{\buff{\mathbf{5}}}\only<8-|handout:0>{\buff{5}}
                    \end{pmatrix}} \\
                    \fakepad{\textcolor{cprimary}{A}}
                \end{array}\)};
              ...
              \onslide<3-7>{
                \draw[Kite-Kite,color=paletteA] (-2.125+\shift,-0.75) to[bend left, edge node={node[above,sloped] {\(\cdot\)}}] (1.325-\nshift,1.65);
              }
              \onslide<4-7>{
                \draw[Kite-Kite,color=paletteB] (-1.5+\shift,-0.75) to[bend left, edge node={node[above,sloped] {\(\cdot\)}}] (1.325-\nshift8,1.15);
              }
              \onslide<5-7>{
                \draw[Kite-Kite,color=paletteC] (-0.99+\shift,-0.75) to[bend left, edge node={node[above,sloped] {\(\cdot\)}}] (1.325-\nshift,0.65);
              }
              \onslide<6-7>{
                \draw[Kite-Kite,color=paletteD] (-0.35+\shift,-0.75) to[bend left, edge node={node[above,sloped] {\(\cdot\)}}] (1.325-\nshift,0.25);
              }
              \node at(6.2,0) {\begin{minipage}{0.4\linewidth}
